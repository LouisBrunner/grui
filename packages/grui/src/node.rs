...
//                 .field("theme_overrides", &element.theme_overrides)
//                 .finish(),
...
//     pub fn with_theme_overrides(mut self, overrides: Option<ThemeOverrides>) -> Self {
...
// /// helpers.
// ///
// /// Omni-specific properties are represented by `OmniProps` and can be applied
// /// to an `ElementBuilder` using `with_omni_props`. This keeps the virtual node
// /// surface pleasant for callers while letting the renderer compose `Omni` into
// /// real container nodes.
// #[derive(Clone, Debug, Default)]
// pub struct OmniProps {
...
//     pub fn direction(mut self, dir: impl Into<PropertyValue>) -> Self {
//         self.props.push(Property::new("direction", dir.into()));
//         self
//     }
...
//         self.theme_overrides = Some(overrides);
//         self
//     }

//     /// Apply `OmniProps` convenience values to this builder (no-op for non-Omni kinds).
//     /// This method records the properties on the builder so the renderer can compose
//     /// the appropriate container subtree when encountering `VNodeKind::Omni`.
//     pub fn with_omni_props(mut self, props: OmniProps) -> Self {
//         if let Some(dir) = props.direction {
//             // use existing `direction` helper which stores a `direction` property
//             self = self.direction(dir);
//         }
//         if let Some(p) = props.padding {
//             self = self.prop("padding", p);
//         }
//         if let Some(s) = props.spacing {
//             self = self.prop("spacing", s);
//         }
//         self
//     }
...
//                 .with_theme_overrides(self.theme_overrides),
//         )
//     }
